...
contract StakingChainCarsTest is Test {
    ...
    function testClaimRewards() public {
        ...
        uint256 expectedRewards = staking.calculateRewards(user1, 0);
        require(
            expectedRewards > 0,
            "Expected rewards should be greater than 0"
        );

        vm.expectEmit(true, true, true, true);
        emit RewardClaimed(user1, TOKEN_ID, expectedRewards);

        staking.claimRewards(0);

        uint256 userBalance = usdt.balanceOf(user1);
        assertEq(userBalance, expectedRewards);
    ...
    function testUnstake() public {
        ...
        vm.expectEmit(true, true, true, true);
        emit Unstaked(user1, TOKEN_ID, STAKE_AMOUNT);
        ...
        assertTrue(info.unstaked);
        
        assertEq(chainCars.balanceOf(user1, TOKEN_ID), STAKE_AMOUNT);

        assertEq(usdt.balanceOf(user1), expectedRewards);
